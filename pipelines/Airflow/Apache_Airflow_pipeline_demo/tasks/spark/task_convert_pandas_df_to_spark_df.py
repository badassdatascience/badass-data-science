...
def task_convert_pandas_df_to_spark_df(**config):
    ...
    sdf_arrays = load_pandas_df_parquet_into_spark_df(
        config['directory_output'] + '/' + config['dag_run'].run_id + '/' + config['filename_finalized_pandas'],
        spark,
    ...
    sdf_arrays.write.mode('overwrite').parquet(config['directory_output'] + '/' + config['dag_run'].run_id + '/' + config['filename_conversion_to_spark'])
